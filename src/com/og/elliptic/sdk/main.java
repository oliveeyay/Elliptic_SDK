...
public class main {
	...
	private static void elGamalString(WeierStrassCurve curve) {
		...
		// Cryptage
		PointGMP generateur = new PointGMP(curve.getGx(), curve.getGy(), curve);// le point générateur qui servira à calculer notre clef
																				// secrète
		Random rnd = new Random();

		BigInteger clefSecrete = BigInteger.probablePrime(7, rnd);
		PointGMP publicKey = generateur.mult(clefSecrete);// on calcule notre clef secrète, qui nous servira à crypter les messages (h)
		...
		PointGMP C1 = generateur.mult(k);// on multiplie le générateur par k
		PointGMP tmp = publicKey.mult(k);
		BigInteger C2 = m.add(tmp.getX());// on multiplie notre clef secrète par k

		// Décryptage
		PointGMP temp = C1.mult(clefSecrete);// on multiplie C1 par notre clef secrète
		BigInteger messageTraduit = C2.subtract(temp.getX());// et on obtient le message en ajoutant m2 avec l'opposé de temp
